...
class ModelArithmetic(PreTrainedModel):
    ...
    def next_token_speculative(self, continuation_tokens, 
        ...
        models_ran = []
        log_prob_dist = []
        # print(f'Runnable Operators : {self.runnable_operators}')
        for i, runnable_operator in enumerate(self.runnable_operators):
            # print(f'Runnable Operator : {runnable_operator}')
            ...
            if runnable_operator.run_condition(new_tokens, self.trigger_end) or not speculation:
                ...
                # print(f'Length of the logprobs : {len(logprobs)}')
                # print(f'Arg Max : {torch.argmax(logprobs[0])}')
                # m = nn.Softmax()
                # output = m(logprobs[0])
                # print(f'Probability Value of the Next Token : {round(output[0][torch.argmax(output)].item(),5)}')
                # log_prob_dist.append(logprobs[0])
                all_kept = self.add_new_result(continuation_tokens, new_tokens, runnable_operator, logprobs, top_p, top_k, temperature)
                # print(f'all_kept : {all_kept}')
                models_ran.append(i)
        ...
        if len(to_sample_indices) > 0:
            ...
            # print(f'All Required Histories : {all_required_histories}')
            new_tokens = self.normal_sample(all_required_histories)
            # print(f'New tokens : {new_tokens[0].item()}')

            # m = nn.Softmax()
            # output = m(log_prob_dist[0])
            # print(f'Probability Value of the Next Token : {round(output[0][torch.argmax(output)].item(),5)}')
            # # output = m(log_prob_dist[1])
            # # print(f'Probability Value of the Next Token : {round(output[0][torch.argmax(output)].item(),5)}')
            # probabilities = torch.softmax(log_prob_dist[0], dim=-1)
            # sorted_probabilities, sorted_indices = torch.sort(probabilities, descending=True)
            # for i in range(10):  # Display top 10 tokens
            #     token_id = sorted_indices[0, i].item()
            #     token_prob = sorted_probabilities[0, i].item()
            #     token = self.tokenizer.decode([token_id])
            #     print(f"Token: {token}, Probability: {token_prob:.8f}")
    ...
    def generate_text(self, sentences, max_new_tokens=1024, stop_texts=None, batch_size=None,
        ...
        # print(self.model_prediction_history)
        # print(self.model_last_token_prediction)
        ...
        #print("Model Input Tokens: ", self.model_input_tokens)                
        total_done = 0
        ...
        while len(current_indices) > 0:
            ...
            # print("generated tokens batch: ", generated_tokens_batch)
            ...
            #print("Next tokens after retroactive operators + clear_model_prediction_history: ", next_tokens)
            ...
            # print("Generated tokens & texts: ", generated_tokens, generated_texts)
            # print("Model prediction history: ", self.model_prediction_history)
            indices_to_remove = []
